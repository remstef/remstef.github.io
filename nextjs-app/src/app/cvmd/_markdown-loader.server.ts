...
export const staticMarkdown: Array<MarkdownFile> = (await Promise.all(
  (await readdir(__subDirname)).map(async (fname) => {
    ...
    const metadataobj = bindex >= 0 && bindex < eindex ? JSON.parse(content.slice(bindex+bmarker.length, eindex).trim()) : {  title: key, pos: -1 }
